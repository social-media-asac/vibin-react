...
export default function Post({ post }) {
  ...
  const updateHandler = async (e,id) => {
    ...
    let value2 =document.getElementById('post');
     value2.innerText = document.getElementById('post').value;
    let postId = post._id;
    // console.log(value,"_))))))))))))))))))))))))))))))))))))))");
    console.log(value2,"_)))))))))/////////////////////)))))))))))))))))))))))))))))");
    let url = `https://vybin.herokuapp.com/api/v1/posts/${id}`;
    await axios.put(url,{"body":null} , { headers: { "Authorization": `Bearer ${token}` } } )
  }
  return (
          ...
          <div className="postWrapper">
            <div className="postTop">
              ...
              <div className="postTopRight">
                ...
                <form type="submit"onSubmit={(e)=>updateHandler(e,post._id)}>
                <button onClick={shareHandler}> Update </button>

                <If condition={boolean}>
                <input id='post' placeholder={post.desc} />
